...
def test_cholesky_with_rhs_warns_deprecation() -> None:
    """Test that the two-argument cholesky form emits a DeprecationWarning."""
    matrix = np.array([[4.0, 0.0], [0.0, 9.0]])
    rhs = np.array([8.0, 27.0])
    with pytest.warns(
        DeprecationWarning,
        match=rf"^{_DEPRECATION_MESSAGE}$",
    ):
...
def test_cholesky_solve_matches_two_arg_cholesky() -> None:
    """Test that cholesky_solve matches the deprecated two-argument cholesky call."""
    a = rand_cov(5, seed=7)
    ...
    with pytest.warns(
        ...
        legacy = cholesky(a, rhs)
    np.testing.assert_allclose(cholesky_solve(a, rhs), legacy)
